...
/*
 ...
 MARK: ***** IMPORTANT ***** 진짜! 무슨말인지! 알것같다! => @Observable객체의 경우, 서로 다른뷰에서 그냥 단순히 옵저빙하면, 그때그때 새로운 인스턴스가 생기는 것이 아닌가??? 그럼 A뷰와 B뷰의 데이터가 달라지는 것이지~!!! 그렇기에 이 객체의 경우, A뷰에서 B뷰로 동일한 인스턴스를 전달해야만 하는 거지~!!! 근데 @Environment객체는 이런것필요없이 모두가 동일한 값을 바라볼수있다는 그런것같다!!!
 
 Environment 객체를 구독하는 객체는 @EnvironmentObject 프로퍼티 래퍼를 이용하여 해당 객체를 참조한다.
 
 TODO: Environment객체는 옵저버 내에서 초기화될 수 없으므로?, 접근하는 뷰가 화면을 설정하는 동안 구성해야 한다!? -> willConnectTo 이야기하는 것 같은데 원리는 아직 잘 모르겠음. -> @ 얘 같은 경우에는 옵저버(즉, @EnvironmentObject로 래핑되어있는 프로퍼티가 있는 뷰)에서 초기화가 불가능하게 정의되어있음. 그러므로, 화면설정단계(초기단계)에서 .environmentObject(...)를 통해 초기화해주어야한다!
